...
class heartspaceNN(object):
    ...
    def channel_wise_gating_mechanism(self, input, filters, name, ratio=1):
        init = input
    ...
    def temporal_gating_mechanism(self, input, filters, name, ratio=1):
        init = input
    ...
    def encoder(self, current_input):
        ...
        for layer_i, n_output in enumerate(self.n_filters):
            ...
            current_input = self.channel_wise_gating_mechanism(current_input, n_output, 'ese_%s' %layer_i, ratio=1)
            current_input = self.temporal_gating_mechanism(current_input, current_input.get_shape().as_list()[1], 'ese_%s' %layer_i, ratio=4)
    ...
    def decoder(self, shapes, current_input):
        ...
        for layer_i, shape in enumerate(shapes):
            ...
            if layer_i!=len(shapes)-1:
                ...
                current_input = self.channel_wise_gating_mechanism(current_input, n_input, 'dse_%s' % layer_i, ratio=1)
                current_input = self.temporal_gating_mechanism(current_input, current_input.get_shape().as_list()[1], 'dse_%s' % layer_i, ratio=4)
            else:
