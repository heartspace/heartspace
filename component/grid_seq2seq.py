...
class seq2seqNN(object):
    ...
    def exp_seq2seq(self, input, keep_prob, rnn_size, output_dim, num_layers):
        output = input
        mask = tf.cast(tf.greater(output, 0.01), tf.float32)
        input_mask = tf.concat([tf.expand_dims(output, axis = -1), tf.expand_dims(mask, axis = -1)], axis = -1)
        enc_outputs, enc_states = self.encoding_layer(input_mask, rnn_size, num_layers, keep_prob)
        ...
        return train_output, bottleneck, loss, opt
...
if __name__ == '__main__':
    ...
    seq2seq = seq2seqNN()
    output, bottleneck, loss, opt = seq2seq.exp_seq2seq(query, keep_prob = 1.0,
                              rnn_size = 64, output_dim = embedding_dim, num_layers = 4)
    init = tf.global_variables_initializer()
    with tf.Session() as sess:
        sess.run(init)
        for i in range(200):
            output_, bottleneck_, loss_, _ = sess.run([output, bottleneck, loss, opt])
            if i%100==0:
                print(np.round(output_,2), np.shape(bottleneck_), loss_)