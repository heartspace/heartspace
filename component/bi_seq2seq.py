# Sleep Stage Classification Based on Multi-level
# Feature Learning and Recurrent Neural Networks
# via Wearable Device
...
class seq2seqNN(object):
    ...
    def encoding_layer(self, rnn_inputs, rnn_size, num_layers, keep_prob):
        fw_cells = [rnn.GRUCell(rnn_size/2) for _ in range(num_layers)]
        bw_cells = [rnn.GRUCell(rnn_size/2) for _ in range(num_layers)]
        ...
        # generate prediction
        rnn_outputs, fw_state, bw_state = rnn.stack_bidirectional_dynamic_rnn(
            fw_cells, bw_cells, rnn_inputs, dtype=tf.float32)
        state = tf.concat([fw_state, bw_state], axis = -1)
        state = tuple(tf.unstack(state, axis = 0))
...
if __name__ == '__main__':
    ...
    output, state_shape, loss, opt = seq2seq.exp_seq2seq(query, keep_prob = 1.0,
                              rnn_size = 64, output_dim = embedding_dim, num_layers = 5)
